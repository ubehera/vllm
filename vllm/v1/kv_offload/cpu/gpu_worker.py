...
class SingleDirectionOffloadingHandler:
    ...
    def __init__(
        ...
        if compact_mode and canonical_layout:
            raise ValueError("compact and canonical CPU layouts are mutually exclusive")
        if not compact_mode and len(gpu_tensors) != len(cpu_tensors):
            raise ValueError("non-compact GPU and CPU tensor counts must match")
        if not gpu_tensors:
    ...
    def _submit_descriptors(
        ...
        *,
        ...
        # Stores wait for model writes to their source; loads wait for pending
        # compute-stream writes (including zeroing) to their destination.
