    ...
    def forward_impl(
        ...
        if self.impl.is_sparse and num_mha_tokens > 0:
            ...
            use_mha = (
                ...
                # homeailab local patch 2026-07-26: route MHA only when the impl
                # actually overrides forward_mha. The DSv32 indexer impl inherits
                # the base-class raise, so pure-prefill batches crashed with
                # NotImplementedError (config docstring says "when implemented").
                and type(self.impl).forward_mha is not MLAAttentionImpl.forward_mha
            )
            if not use_mha:
