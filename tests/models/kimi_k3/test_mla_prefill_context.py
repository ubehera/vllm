...
"""K3's fused aggregate chunked-context prefill matches the generic MLA impl."""
...
class _RecordingPrefillBackend:
    """Record what each aggregate chunk is asked to attend over."""
...
def test_fused_context_matches_generic_impl(
    kv_b_proj_quantized: bool, kv_cache_dtype: str
) -> None:
    ...
    backend_fused = _RecordingPrefillBackend()
    ...
    metadata_fused = SimpleNamespace(
        prefill=prefill_fused, num_prefills=len(_QUERY_LENS)
    )
    metadata_ref = SimpleNamespace(
        prefill=prefill_ref, num_prefills=len(_QUERY_LENS)
    )

    layer = _FusedLayer(kv_b_proj, kv_cache_dtype)
    fused_out, fused_lse = layer._compute_prefill_context(
        q, kv_cache, metadata_fused, k_scale
    )

    impl = _ReferenceImpl(kv_b_proj, kv_cache_dtype)
    ref_out, ref_lse = impl._compute_prefill_context(
        q, kv_cache, metadata_ref, k_scale
    )
...
def test_fused_context_rejects_an_unquantized_query() -> None:
    ...
    kv_cache = torch.zeros(
        (num_blocks, _BLOCK_SIZE, _ENTRY), device=device, dtype=fp8
    )
    k_scale = torch.ones(1, dtype=torch.float32, device=device)
    q = torch.zeros(
        (sum(_QUERY_LENS), _NUM_HEADS, _QK_NOPE + _QK_ROPE),
    ...
    with pytest.raises(AssertionError, match="new-token epilogue"):
        layer._compute_prefill_context(
            q,
            kv_cache,
            SimpleNamespace(prefill=prefill, num_prefills=len(_QUERY_LENS)),
            k_scale,
        )
