...
def test_prefix_cache_peek_does_not_record_stats():
    ...
    # A lookup on its own never records: stats are recorded only by an explicit
    # record_prefix_cache_stats() call (upstream moved recording out of
    # get_computed_blocks, replacing the fork's record_stats=False peek).
    _, peeked_tokens, _ = scheduler.kv_cache_manager.get_computed_blocks(replay_req)
    ...
    _, recorded_tokens, _ = scheduler.kv_cache_manager.get_computed_blocks(replay_req)
    assert recorded_tokens == peeked_tokens
    scheduler.kv_cache_manager.record_prefix_cache_stats(replay_req, recorded_tokens)
    stats = scheduler.kv_cache_manager.make_prefix_cache_stats()
    assert stats is not None
