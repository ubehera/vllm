  ...
  if (use_batch_api) {
    ...
    if (batch_fn != nullptr && usable_stream) {
      ...
      for (int64_t off = 0; off < n; off += step) {
        ...
        CUresult result = batch_fn(
            reinterpret_cast<CUdeviceptr*>(dst_data + off),
            reinterpret_cast<CUdeviceptr*>(src_data + off),
            reinterpret_cast<size_t*>(size_data + off),
            static_cast<size_t>(cnt), &attr, &attrs_idx, 1, &fail_idx,
    ...
    // ROCm 7.1+ exposes hipMemcpyBatchAsync. ROCm 7.2.1-7.2.3 reject attrs;
    // ROCm 7.13+ accepts them and can honor the requested source ordering.
    {
      ...
      for (int64_t off = 0; off < n; off += step) {
        ...
        hipError_t result = hipMemcpyBatchAsync(
            ...
            reinterpret_cast<size_t*>(size_data + off),
            static_cast<size_t>(cnt), &attr, &attrs_idx, num_attrs, &fail_idx,
